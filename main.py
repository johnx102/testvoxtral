            ...
            if ENABLE_HOLD_MUSIC_DETECTION:
                try:
                    ...
                    if data.ndim > 1:
                        ...
                        if hold_regions_ch0:
                            total = sum(e - s for s, e in hold_regions_ch0)
                            log(f"[HOLD_REGIONS] CH0 (Client): {len(hold_regions_ch0)} zones, {total:.1f}s")
                        if hold_regions_ch1:
                            total = sum(e - s for s, e in hold_regions_ch1)
                            log(f"[HOLD_REGIONS] CH1 (Agent): {len(hold_regions_ch1)} zones, {total:.1f}s")
                    del data
                except Exception as e:
