...
# ============================================================================
# NOUVELLES FONCTIONS: Résumé et Sentiment
# ============================================================================

def generate_summary(text: str, language: str = "fr") -> str:
    """
    Génère un résumé extractif de la transcription.
    Extrait les phrases clés du début, milieu et fin.
    """
    ...
    try:
        ...
        # Prendre les phrases clés: début (2), milieu (1-2), fin (2)
        summary_parts = []
        ...
        if mid_end > mid_start:
            summary_parts.append(sentences[(mid_start + mid_end) // 2])
...
def analyze_sentiment(text: str, language: str = "fr") -> dict:
    ...
    Retourne sentiment, score et satisfaction client.
    """
    if not text:
        return {"sentiment": "neutre", "score": 0.5, "satisfaction_client": "neutre"}
    ...
    sentiment_words = {
        "fr": {
            "positive": [
                ...
                "heureux", "plaisir", "réussi", "succès", "efficace", "rapide", "aide",
                "résolu", "solution", "comprends", "clair", "facile", "géniale", "top",
                "impeccable", "nickel", "cool", "super", "magnifique", "fantastique"
            ],
            "negative": [
                ...
                "jamais", "rien", "non", "refus", "annuler", "inadmissible", "scandaleux",
                "inacceptable", "catastrophe", "désastre", "honteux", "lamentable"
            ]
        },
        "en": {
            "positive": [
                "thank", "thanks", "perfect", "excellent", "great", "good", "well",
                "happy", "satisfied", "wonderful", "amazing", "yes", "agree", "nice",
                "pleased", "success", "helpful", "resolved", "clear", "easy", "awesome"
            ],
            "negative": [
                "problem", "error", "bad", "wrong", "unsatisfied", "disappointed", "terrible",
                "horrible", "worst", "difficult", "complicated", "impossible", "failure",
                "complaint", "angry", "frustrated", "wait", "long", "incompetent", "never",
                "nothing", "no", "refuse", "cancel", "unacceptable", "disaster"
            ]
        }
    }
    
    # Sélectionner le dictionnaire approprié
    lang_key = language[:2].lower() if language else "fr"
    if lang_key not in sentiment_words:
        lang_key = "fr"  # Fallback
    
    positive = sentiment_words[lang_key]["positive"]
    negative = sentiment_words[lang_key]["negative"]
    ...
    if total == 0:
        ...
        sentiment = "positif"
        score = min(0.95, 0.5 + (pos_count - neg_count) / (total + 5) * 0.5)
    elif neg_count > pos_count * 1.5:
        sentiment = "négatif"
        score = max(0.05, 0.5 - (neg_count - pos_count) / (total + 5) * 0.5)
    else:
        sentiment = "neutre"
        score = 0.5 + (pos_count - neg_count) / (total + 10) * 0.3
    
    # Déterminer la satisfaction client
    if score >= 0.65:
...
def extract_full_text(segments: list) -> str:
    """
    Extrait le texte complet depuis la liste des segments.
    """
    if not segments:
        return ""
...
def run(job):
    ...
    # ------------- Extraire les options résumé/sentiment ------------
    do_summary = job_input.get("summary", False)
    do_sentiment = job_input.get("sentiment", False)
    
    if do_summary or do_sentiment:
        logger.info(f"[OPTIONS] summary={do_summary}, sentiment={do_sentiment}")
    ...
    if speaker_profiles:
        try:
            ...
            # Note: output_dict not defined yet, will handle in output
    ...
    # ============================================================================
    # 5) RÉSUMÉ ET SENTIMENT (NOUVEAU)
    # ============================================================================
    
    if do_summary or do_sentiment:
        # Extraire le texte complet
        full_text = extract_full_text(output_dict.get("segments", []))
        detected_lang = output_dict.get("detected_language", "fr")
        
        if do_summary and full_text:
            try:
                output_dict["summary"] = generate_summary(full_text, detected_lang)
                logger.info("[SUMMARY] Summary generated successfully")
            except Exception as e:
                logger.error(f"[SUMMARY] Failed: {e}", exc_info=True)
                output_dict["summary"] = ""
        
        if do_sentiment and full_text:
            try:
                output_dict["sentiment"] = analyze_sentiment(full_text, detected_lang)
                logger.info("[SENTIMENT] Sentiment analysis completed successfully")
            except Exception as e:
                logger.error(f"[SENTIMENT] Failed: {e}", exc_info=True)
                output_dict["sentiment"] = {"sentiment": "neutre", "score": 0.5, "satisfaction_client": "neutre"}
    
    # ============================================================================
    # FIN RÉSUMÉ ET SENTIMENT
    # ============================================================================
