...
# Whisper : modèle FORCÉ EN DUR (env var ignorée pour résister aux setup
# RunPod où WHISPER_MODEL_SIZE est bloqué à une autre valeur).
#
...
_RAW_WHISPER_MODEL = os.environ.get("WHISPER_MODEL_SIZE", "").strip()
if _RAW_WHISPER_MODEL and _RAW_WHISPER_MODEL != WHISPER_MODEL_SIZE:
    print(f"[CONFIG] WHISPER_MODEL_SIZE='{_RAW_WHISPER_MODEL}' (env) ignoré → forcé '{WHISPER_MODEL_SIZE}'", flush=True)
...
def _download_bofenghuang_french_model() -> str:
    """Télécharge le sous-dossier ctranslate2/ du modèle bofenghuang français.

    Modèle : bofenghuang/whisper-large-v3-french-distil-dec16
    - Distillé (16 décodeurs au lieu de 32) → ~2× plus rapide que large-v3
    - Fine-tuné sur 5 datasets français
    - WER 7.18% sur CommonVoice (vs ~10% pour large-v3 standard)
    - Réduit les hallucinations sur les transcriptions longues

    Retourne le chemin local du dossier ctranslate2/ prêt à charger.
    """
    from huggingface_hub import snapshot_download
    repo_id = "bofenghuang/whisper-large-v3-french-distil-dec16"
    local_dir = "/app/.cache/whisper-french-distil-dec16"
    log(f"[WHISPER] Downloading bofenghuang french model (ctranslate2 subfolder)...")
    snapshot_download(
        repo_id=repo_id,
        local_dir=local_dir,
        allow_patterns="ctranslate2/*",
    )
    ct2_path = os.path.join(local_dir, "ctranslate2")
    log(f"[WHISPER] French model ready at: {ct2_path}")
    return ct2_path
...
def _load_whisper():
    ...
    try:
        ...
        # Active avec WHISPER_MODEL_SIZE="bofenghuang-french" ou "french"
        model_id = WHISPER_MODEL_SIZE
        if model_id.lower() in ("bofenghuang-french", "french", "fr-distil"):
            log(f"[WHISPER] Using bofenghuang french distilled model")
            ct2_path = "/app/.cache/whisper-french-distil-dec16/ctranslate2"
            if not os.path.exists(os.path.join(ct2_path, "model.bin")):
                ct2_path = _download_bofenghuang_french_model()
            else:
                log(f"[WHISPER] French model cached at {ct2_path}")
            _whisper_model = WhisperModel(
                ct2_path,
                device=WHISPER_DEVICE,
                compute_type=WHISPER_COMPUTE,
            )
            log("[WHISPER] Model loaded successfully (bofenghuang french distil dec16)")
            return _whisper_model

        # Cas standard : modèles Systran/faster-whisper-* (large-v2, large-v3, etc.)
        log(f"[WHISPER] Loading model: {model_id} (device={WHISPER_DEVICE}, compute={WHISPER_COMPUTE})")
